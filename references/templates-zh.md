...
仅在跨模块、多阶段、异步执行、状态流转、fallback 或 Agent handoff 难以通过短列表说明时加入。通常放在“本 PR 完成了什么”之后、“Commit 记录”之前。
...
flowchart TD
    ...
    D --> E{"执行是否成功"}
    E -->|"成功"| F["保存结果并通知用户"]
    E -->|"失败"| G["记录失败并执行 fallback"]
```
````

图中只能描述最终 diff 已实现且有证据支持的流程。简单 PR 不要为了完整模板而加入图示。
